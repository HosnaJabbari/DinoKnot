...
double W_final::hfold_interacting() {
    ...
    for (j=0; j < nb_nucleotides; j++)
    {
        for (i=0; i<j; i++)
        {
            ...
            if (fres[i].pair == -1 || fres[j].pair == -1)   // i or j MUST be unpaired
            continue;
            V->compute_energy_restricted_pmo (i, j, fres);
        }
    ...
    for (j= 1; j < nb_nucleotides; j++) {
        this->compute_W_restricted_pmo(j,fres);
    }
    energy = this->W[nb_nucleotides-1]/100.0;
    ...
    while (cur_interval != NULL) {
        stack_interval = stack_interval->next;
        backtrack_restricted_pmo (cur_interval, fres);
        delete cur_interval;    // this should make up for the new in the insert_node
        cur_interval = stack_interval;
    }
...
double W_final::hfold_interacting_pkonly() {
    ...
    for (j=0; j < nb_nucleotides; j++)
    {
        for (i=0; i<j; i++)
        {
            ...
            if (fres[i].pair == -1 || fres[j].pair == -1)   // i or j MUST be unpaired
            continue;
            V->compute_energy_restricted_pkonly_pmo (i, j, fres);
        }
    ...
    while (cur_interval != NULL) {
        stack_interval = stack_interval->next;
        backtrack_restricted_pkonly_pmo (cur_interval, fres);
        delete cur_interval;    // this should make up for the new in the insert_node
        cur_interval = stack_interval;
    }
...
void W_final::compute_W_restricted_pmo (int j, str_features *fres)
// compute W(j)
{
    int m1, m2, m3;
    int must_choose_this_branch;
    m1 = W[j-1];
    m2 = compute_W_br2_restricted_pmo (j, fres, must_choose_this_branch);
    m3 = compute_W_br3_restricted (j, fres);
    if (WMB->is_weakly_closed(0,j) < 0){
    	W[j] = INF;
    	return;
...
// compute W(j)
{
    ...
    int must_choose_this_branch;
    m1 = W[j-1];
    m2 = compute_W_br2_restricted_pkonly_pmo (j, fres, must_choose_this_branch);
    m3 = compute_W_br3_restricted (j, fres);
    if (WMB->is_weakly_closed(0,j) < 0){
    	W[j] = INF;
    	return;
...
int W_final::compute_W_br2_restricted_pmo (int j, str_features *fres, int &must_choose_this_branch) {
	int min = INF, tmp, energy_ij = INF, acc;
    int i;
    int chosen = 0;
    int best_i = 0;
    ...
    for (i=0; i<=j-1; i++) {
        ...
        if (energy_ij < INF)
        {
            tmp = energy_ij + acc + (PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i],int_sequence[j]) + rna_percentage*AU_penalty(int_sequence[i],int_sequence[j]));
            if (tmp < min)
            {
                min = tmp;
                chosen = 21;
        ...
        if (fres[i].pair <= -1 && i+1 < j)
        {
            ...
            if (energy_ij < INF)
            {
                tmp = energy_ij + acc + (PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i+1],int_sequence[j]) + rna_percentage*AU_penalty(int_sequence[i+1],int_sequence[j]));

                PARAMTYPE dan = (PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j]]
																		[int_sequence[i+1]]
																		[int_sequence[i]] +
        ...
        if (fres[j].pair <= -1 && i < j-1)
        {
            ...
            if (energy_ij < INF)
            {
                tmp = energy_ij + acc + (PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i],int_sequence[j-1]) + rna_percentage*AU_penalty(int_sequence[i],int_sequence[j-1]));

				PARAMTYPE dan = (PARAMTYPE) (pmo_percentage*dangle_top_pmo[int_sequence[j-1]]
																		[int_sequence[i]]
																		[int_sequence[j]] +
        ...
        if (fres[i].pair <= -1 && fres[j].pair <= -1 && i+1 < j-1)
        {
            ...
            if (energy_ij < INF)
            {
                tmp = energy_ij + acc + (PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i+1],int_sequence[j-1]) + rna_percentage*AU_penalty(int_sequence[i+1],int_sequence[j-1]));

				PARAMTYPE dan_bot = (PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j-1]]
																			[int_sequence[i+1]]
																			[int_sequence[i]] +
				...
				PARAMTYPE dan_top = (PARAMTYPE) (pmo_percentage*dangle_top_pmo[int_sequence[j-1]]
																			[int_sequence[i+1]]
																			[int_sequence[j]] +
													rna_percentage*dangle_top[int_sequence[j-1]]
																			[int_sequence[i+1]]
...
int W_final::compute_W_br2_restricted_pkonly_pmo (int j, str_features *fres, int &must_choose_this_branch)
{
	int min = INF, tmp, energy_ij = INF, acc;
    int i;
    int chosen = 0;
			...
			//else if (type == INTER)
				{
					...
					for (ip = i+1; ip <= MIN(j-2,i+MAXLOOP+1); ip++)
					{
						...
						for (jp = minq; jp < j; jp++)
						{
							...
							// Hosna, March 26, 2012
							// modified to accommodate non-canonical base pairing in restricted structure
							tmp = VBI->get_energy_str_restricted_pmo(i,j,ip,jp,fres);
							if (tmp < min)
							{
								min = tmp;
								best_ip = ip;
					...
					for (k = i+1; k <= j-1; k++)
					  {
						...
						if (fres[i+1].pair <= -1)
						{
							tmp = vm->get_energy_WM (i+2,k) +
								vm->get_energy_WM (k+1, j-1) +
								(PARAMTYPE) (pmo_percentage*dangle_top_pmo[int_sequence[i]]
																		[int_sequence[j]]
																		[int_sequence[i+1]] +
						...
						if (fres[j-1].pair <= -1)
						{
							tmp = vm->get_energy_WM (i+1,k) +
								vm->get_energy_WM (k+1, j-2) +
								(PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[i]]
																		[int_sequence[j]]
																		[int_sequence[j-1]] +
			...
			for (i=0; i<=j-1; i++)    // no TURN
			{
				...
				if (fres[i].pair <= -1)
				{
					...
					if (energy_ij < INF)
					{
						...
						tmp += (PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j]]
																		[int_sequence[i+1]]
																		[int_sequence[i]] +
												rna_percentage*dangle_bot[int_sequence[j]]
																		[int_sequence[i+1]]
				...
				if (fres[j].pair <= -1)
				{
					...
					if (energy_ij < INF)
					{
						...
						tmp += (PARAMTYPE) (pmo_percentage*dangle_top_pmo[int_sequence[j-1]]
																		[int_sequence[i]]
																		[int_sequence[j]] +
												rna_percentage*dangle_top[int_sequence[j-1]]
																		[int_sequence[i]]
				...
				if (fres[i].pair <= -1 && fres[j].pair <= -1)
				{
					...
					if (energy_ij < INF)
					{
						...
						tmp += (PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j-1]]
																		[int_sequence[i+1]]
																		[int_sequence[i]] +
												rna_percentage*dangle_bot[int_sequence[j-1]]
																		[int_sequence[i+1]]
...
//  else if(cur_interval->type == M_WM)
		{
			...
			tmp = v->get_energy(i,j) +
				(PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i], int_sequence[j]) + rna_percentage*AU_penalty(int_sequence[i], int_sequence[j])) +
			...
			{
				tmp = v->get_energy(i+1,j) +
					(PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i+1], int_sequence[j]) + rna_percentage*AU_penalty(int_sequence[i+1], int_sequence[j])) +
					(PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j]]
															[int_sequence[i+1]]
															[int_sequence[i]] +
									rna_percentage*dangle_bot[int_sequence[j]]
															[int_sequence[i+1]]
			...
			{
				tmp = v->get_energy(i,j-1) +
					(PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i], int_sequence[j-1]) + rna_percentage*AU_penalty(int_sequence[i], int_sequence[j-1])) +
					(PARAMTYPE) (pmo_percentage*dangle_top_pmo[int_sequence[j-1]]
															[int_sequence[i]]
															[int_sequence[j]] +
									rna_percentage*dangle_top[int_sequence[j-1]]
															[int_sequence[i]]
			...
			{
				tmp = v->get_energy(i+1,j-1) +
					(PARAMTYPE) (pmo_percentage*AU_penalty_pmo(int_sequence[i+1], int_sequence[j-1]) + rna_percentage*AU_penalty(int_sequence[i+1], int_sequence[j-1])) +
					(PARAMTYPE) (pmo_percentage*dangle_bot_pmo[int_sequence[j-1]]
															[int_sequence[i+1]]
															[int_sequence[i]] +
									rna_percentage*dangle_bot[int_sequence[j-1]]
															[int_sequence[i+1]]
			...
			if (fres[i].pair <= -1)
			{
				tmp = vm->get_energy_WM (i+1,j) + (PARAMTYPE) (pmo_percentage*misc_pmo.multi_free_base_penalty + rna_percentage*misc.multi_free_base_penalty);
				if (tmp < min)
				{
					min = tmp;
					best_row = 5;
			...
			if (fres[j].pair <= -1)
			{
				tmp = vm->get_energy_WM (i,j-1) + (PARAMTYPE) (pmo_percentage*misc_pmo.multi_free_base_penalty + rna_percentage*misc.multi_free_base_penalty);
				if (tmp < min)
				{
					min = tmp;
					best_row = 6;
   ...
   // else if(cur_interval->type == P_WMB)
		{
			WMB->set_stack_interval(stack_interval);
			WMB->back_track_pmo(structure,f,cur_interval);
			stack_interval = WMB->get_stack_interval();
			structure = WMB->get_structure();
			f = WMB->get_minimum_fold();
		}
...
void W_final::backtrack_restricted_pkonly_pmo (seq_interval *cur_interval, str_features *fres){
	...
	switch (cur_interval->type){
					...
					//else if (type == INTER)
				{
					...
					for (ip = i+1; ip <= j-2 ; ip++)  // the -TURN shouldn't be there
					{
						...
						for (jp = minq; jp < j; jp++)
						{
							...
							// April 18, 2012
							// added a condition for pkonly
							tmp = (fres[ip].pair == jp && fres[jp].pair == ip)? VBI-> get_energy_str_restricted_pmo (i,j,ip,jp,fres):INF;
							if (tmp < min)
							{
								min = tmp;
								best_ip = ip;
			...
			// else if(cur_interval->type == P_WMB)
		{
			...
			// Hosna, May 1st, 2012
			// changed to back_track_pkonly instead of back_track to accommodate pkonly case
			WMB->back_track_pkonly_pmo(structure,f,cur_interval);
			stack_interval = WMB->get_stack_interval();
			structure = WMB->get_structure();
			f = WMB->get_minimum_fold();
		}
