...
/*    u = new double [total_length];
    ...
    unod5 = new double [total_length];
    if (unod5 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    unod3 = new double [total_length];
    if (unod3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    unod35 = new double [total_length];
    if (unod35 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s1 = new double [total_length];
    if (s1 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s1nod5 = new double [total_length];
    if (s1nod5 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s1nod3 = new double [total_length];
    if (s1nod3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s1nod35 = new double [total_length];
    if (s1nod35 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s2 = new double [total_length];
    if (s2 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s2nod3 = new double [total_length];
    if (s2nod3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    u1 = new double [total_length];
    if (u1 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    u1nod5 = new double [total_length];
    if (u1nod5 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    u1nod3 = new double [total_length];
    if (u1nod3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    u1nod35 = new double [total_length];
    if (u1nod35 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s3 = new double [total_length];
    if (s3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s3nod5 = new double [total_length];
    if (s3nod5 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s3nod3 = new double [total_length];
    if (s3nod3 == NULL) giveup ("Cannot allocate memory", "s_partition_function");

    s3nod35 = new double [total_length];
    if (s3nod35 == NULL) giveup ("Cannot allocate memory", "s_partition_function");   */
...
//             compute_u  (i, j);    // must be after up
//             compute_u1 (i, j);    // must be after up
//             compute_u1nod5 (i, j);    // must be after up
//             compute_u1nod3 (i, j);    // must be after up
//             compute_u1nod35 (i, j);    // must be after up
...
//             compute_unod5  (i, j);    // must be after up
//             compute_unod3  (i, j);    // must be after up
//             compute_unod35  (i, j);    // must be after up
//             compute_s1 (i, j);    // doesn't matter where it is, all dependencies have been computed at previous steps
//             compute_s1nod5 (i, j);    // doesn't matter where it is
//             compute_s1nod3 (i, j);    // doesn't matter where it is
//             compute_s1nod35 (i, j);    // doesn't matter where it is
//             compute_s2 (i, j);    // doesn't matter where it is, all dependencies have been computed at previous steps
//             compute_s2nod3 (i, j);    // doesn't matter where it is, all dependencies have been computed at previous steps
//             compute_s3 (i, j);    // doesn't matter where it is, all dependencies have been computed at previous steps
//             compute_s3nod5 (i, j);
//             compute_s3nod3 (i, j);
//             compute_s3nod35 (i, j);
...
void s_partition_function::compute_u (int i, int j)
{
    ...
    u[ij] = 1 + up[ij] * exp_AUpenalty (i,j);    // case (...)

    for (h = i+1; h < j; h++)                    // case ...(...)
    {
        if (can_pair (sequence[h], sequence[j]))    // should be faster
        {
            hj = index[h]+j-h;

            // we could consider misc.hairpin_GGG separately from the hairpin loop, but that's only to have the exhaustive u's and up's exactly the same as the smart ones
            // see if we have to edd misc.hairpin_GGG
            //
            //double GGG = 0;
            //if (j-h-1 == 3 && i <= h-2)
            //{
            //    if (sequence[h-2] == G && sequence[h-1] == G &&
            //        sequence[h] == G && sequence[j] == U)
            //    GGG = misc.hairpin_GGG;
            //}

            if (ignore_dangles)
                u[ij] += up[hj] * exp_AUpenalty (h, j);    // * exp(GGG*oneoverRT);
            else
                u[ij] += up[hj] * exp_dangle5 (j, h, h-1) * exp_AUpenalty (h, j);
        }
    }

    for (l = i+1; l < j; l++)                    // case (...)...---
    {
        il = index[i]+l-i;
        if (l+2 < j)
        {
            if (can_pair (sequence[i], sequence[l]))
            {
                lp2j = index[l+2]+j-l-2;
                lp1j = index[l+1]+j-l-1;
                if (ignore_dangles)
                    u[ij] += up[il] * exp_AUpenalty (i, l) * u[lp1j];
                else
                {
                    u[ij] += up[il] * exp_AUpenalty (i, l) * (exp_dangle3 (l, i, l+1) * u[lp2j] + unod5[lp1j] - unod5[lp2j]);
                    //u[ij] += up[il] * exp_AUpenalty (i, l) * (exp_dangle3 (l, i, l+1) * u[lp2j]);
                    //u[ij] += up[il] * exp_AUpenalty (i, l) * (unod5[lp1j] - unod5[lp2j]);
                }
            }
        }
        ...
        else
        {
            ...
            {
                if (ignore_dangles)
                    u[ij] += up[il] * exp_AUpenalty (i, l);
                else
                    u[ij] += up[il] * exp_AUpenalty (i, l) * exp_dangle3 (l, i, l+1);
            }
        }
...
//         if (ignore_dangles)
//             upm[ij] += exp_AUpenalty (i,j) * up[ip1l] * EXPA * EXPB(2) *
//                         exp_AUpenalty (i+1,l) *
//                         u1[lp1jm1];
//         else
            upm[ij] += exp_AUpenalty (i,j) * up[ip1l] * EXPA * EXPB(2) *
...
//         if (ignore_dangles)
//             upm[ij] += exp_AUpenalty (i,j) * up[ip2l] * EXPA * EXPB(2) * EXPC(1) *
//                     exp_AUpenalty (i+2, l) *
//                     u1[lp1jm1];
//         else
            upm[ij] += exp_AUpenalty (i,j) * up[ip2l] * EXPA * EXPB(2) * EXPC(1) *
...
//         if (ignore_dangles)
//             upm_temp += EXPA * EXPB(2) * EXPC(h-i-1) * s2[hjm1];
//         else
            upm_temp += EXPA * EXPB(2) * EXPC(h-i-1) *
                ( s2_jp[hjm1]    // --(...)) or --(...).)
...
// has at least 2 branches, and h is paired with some l in between h and j
{
    ...
    for (l = h+1; l < j-3; l++)    // .(...)
    {
        ...
        if (ignore_dangles)
            ...
            s2[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                    ( exp_dangle3 (l, h, l+1) * EXPC(1)*u1[lp2j] + u1nod5[lp1j] - EXPC(1)*u1nod5[lp2j] );
    }
}


void s_partition_function::compute_s2nod3 (int h, int j)
// helper for computing upm, which closes a multi-loop
// modified from Ding and Lawrence, to be able to add the right most d5 dangling end: (--(...)..)
{
    if (ignore_dangles)
    ...
    for (l = h+1; l < j-3; l++)    // .(...)
    {
        ...
        lp2j = index[l+2]+j-l-2;
        lp1j = index[l+1]+j-l-1;
        s2nod3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                  ( exp_dangle3 (l, h, l+1) * EXPC(1)*u1nod3[lp2j] + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j] );
...
// contains at least one branch of a multi-loop
{
    ...
    for (l=i+1; l <= j; l++)    // (...)----
    {
        il = index[i]+l-i;        // (...)....
        if (ignore_dangles)
            u1[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) * EXPC(j-l);
        else
            u1[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) *
        ...
        if (l+2 < j)            // (...)-(--)-
        {
            ...
            lp2j = index[l+2]+j-l-2;
            if (ignore_dangles)
                u1[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) * u1[lp1j];
            else
                u1[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) *
                            (exp_dangle3 (l, i, l+1)*u1[lp2j]*EXPC(1) + u1nod5[lp1j] - EXPC(1)*u1nod5[lp2j]);
        }
    }
    for (l=i+2; l <= j; l++)    // .(...)---
    {
        ip1l = index[i+1]+l-i-1;    // .(...)....
        IFD
            u1[ij] += up[ip1l] * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) * EXPC(j-l);
        else
            u1[ij] += up[ip1l] * exp_dangle5 (l, i+1, i) * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) * ( fd3(j+1,i+1,l) * EXPC(j-l) );
        ...
        if (l+2 < j)                  // .(...)----
        {
            ...
            lp2j = index[l+2]+j-l-2;
            IFD
                u1[ij] += up[ip1l] * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) * u1[lp1j];
            else
                u1[ij] += up[ip1l] * exp_dangle5 (l, i+1, i) * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) *
                    ( exp_dangle3 (l, i+1, l+1)*u1[lp2j]*EXPC(1) + u1nod5[lp1j] - EXPC(1)*u1nod5[lp2j]);
        }
    }
    for (h=i+2; h <= j-1; h++)    // ...(...)---
    {
        ...
        u1[ij] += EXPB(1) * EXPC(h-i) * s3[hj];
    }
}


void s_partition_function::compute_u1nod5 (int i, int j)
// contains at least one branch
{
    ...
    {
        il = index[i]+l-i;        // (...)....
        u1nod5[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) *
                  ( fd3(j+1,i,l) * EXPC(j-l) );
    ...
    {
        ip1l = index[i+1]+l-i-1;    // .(...)....
        u1nod5[ij] += up[ip1l] * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) *
    ...
    for (h=i+2; h <= j-1; h++)    // ...(...)---
    {
        hj = index[h]+j-h;
        // d5 is included in s3, but helix penalty is not
        u1nod5[ij] += EXPB(1) * EXPC(h-i) * s3nod5[hj];
    }
}


void s_partition_function::compute_u1nod3 (int i, int j)
// contains at least one branch
{
    IFD return;
    ...
    {
        il = index[i]+l-i;        // (...)....
        u1nod3[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) * EXPC(j-l);

        if (l+2 < j)            // (...)----
        {
            lp1j = index[l+1]+j-l-1;
            lp2j = index[l+2]+j-l-2;
            u1nod3[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) *
                        (exp_dangle3 (l, i, l+1) * u1nod3[lp2j]*EXPC(1) + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j]);
        }
    }
    for (l=i+2; l <= j; l++)    // .(...)---
    {
        ip1l = index[i+1]+l-i-1;    // .(...)....
        u1nod3[ij] += up[ip1l] * exp_dangle5 (l, i+1, i) * EXPB(1) * EXPC(1) *  exp_AUpenalty(i+1,l) * EXPC(j-l);

        if (l+2 < j)                  // .(...)----
        {
            lp1j = index[l+1]+j-l-1;
            lp2j = index[l+2]+j-l-2;
            u1nod3[ij] += up[ip1l] * exp_dangle5 (l, i+1, i) * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) *
                    ( exp_dangle3 (l, i+1, l+1) * u1nod3[lp2j]*EXPC(1) + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j]);
        }
    }
    for (h=i+2; h <= j-1; h++)    // ...(...)---
    {
        hj = index[h]+j-h;
        // d5 is included in s3, but helix penalty is not
        u1nod3[ij] += EXPB(1) * EXPC(h-i) * s3nod3[hj];
    }
}


void s_partition_function::compute_u1nod35 (int i, int j)
// contains at least one branch
{
    IFD return;
    ...
    {
        il = index[i]+l-i;        // (...)....
        u1nod35[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) * EXPC(j-l);

        if (l+2 < j)            // (...)----
        {
            lp1j = index[l+1]+j-l-1;
            lp2j = index[l+2]+j-l-2;
            u1nod35[ij] += up[il] * EXPB(1) * exp_AUpenalty(i,l) *
                        (exp_dangle3 (l, i, l+1) * u1nod3[lp2j]*EXPC(1) + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j]);
        }
    }
    for (l=i+2; l <= j; l++)    // .(...)---
    {
        ip1l = index[i+1]+l-i-1;    // .(...)....
        u1nod35[ij] += up[ip1l] * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) * EXPC(j-l);

        if (l+2 < j)                  // .(...)----
        {
            lp1j = index[l+1]+j-l-1;
            lp2j = index[l+2]+j-l-2;
            u1nod35[ij] += up[ip1l] * EXPB(1) * EXPC(1) * exp_AUpenalty(i+1,l) *
                    ( exp_dangle3 (l, i+1, l+1) * u1nod3[lp2j]*EXPC(1) + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j]);
        }
    }
    for (h=i+2; h <= j-1; h++)    // ...(...)---
    {
        hj = index[h]+j-h;
        // d5 is included in s3, but helix penalty is not
        u1nod35[ij] += EXPB(1) * EXPC(h-i) * s3nod35[hj];
    }
}
*/
    ...
    for (l = h+1; l < j; l++)        // .(...)---
    {
        ...
        if (l+2 < j)
        {
            ...
            IFD
                ...
                s1[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                    ( exp_dangle3 (l, h, l+1) * u[lp2j] + unod5[lp1j] - unod5[lp2j] );
        }
        else
        {
            IFD
                ...
                s1[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) * exp_dangle3 (l, h, l+1);

        }
    }
}

void s_partition_function::compute_s1nod5 (int h, int j)
{
    if (h < 1)  return;
    IFD return;
    ...
    for (l = h+1; l < j; l++)        // .(...)---
    {
        ...
        if (l+2 < j)
        {
            ...
            s1nod5[hj] += up[hl] * exp_AUpenalty (h, l) *
                    //( exp_dangle3 (l, h, l+1) * u[lp2j] + u[lp1j] - u[lp2j] );
                    ( exp_dangle3 (l, h, l+1) * u[lp2j] + unod5[lp1j] - unod5[lp2j] );
        }
        else
        {
            s1nod5[hj] += up[hl] * exp_AUpenalty (h, l) * exp_dangle3 (l, h, l+1);
...
void s_partition_function::compute_s1nod3 (int h, int j)
{
    if (h < 1)  return;
    IFD return;
    ...
    for (l = h+1; l < j; l++)    // .(...)---
    {
        ...
        if (l+2 < j)
        {
            ...
            s1nod3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                    //( exp_dangle3 (l, h, l+1) * u[lp2j] + u[lp1j] - u[lp2j] );
                    //( exp_dangle3 (l, h, l+1) * u[lp2j] + unod5[lp1j] - unod5[lp2j] );
                    ( (unod3[lp2j]>1?exp_dangle3 (l, h, l+1):1) * unod3[lp2j] +
                       unod35[lp1j] - unod35[lp2j] );
        }
        else
        {
            s1nod3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l);
...
void s_partition_function::compute_s1nod35 (int h, int j)
{
    if (h < 1)  return;
    IFD return;
    ...
    for (l = h+1; l < j; l++)    // .(...)---
    {
        ...
        if (l+2 < j)
        {
            ...
            s1nod35[hj] += up[hl] * exp_AUpenalty (h, l) *
                    //( exp_dangle3 (l, h, l+1) * u[lp2j] + u[lp1j] - u[lp2j] );
                    //( exp_dangle3 (l, h, l+1) * u[lp2j] + unod5[lp1j] - unod5[lp2j] );
                    ( (unod3[lp2j]>1?exp_dangle3 (l, h, l+1):1) * unod3[lp2j] +
                       unod35[lp1j] - unod35[lp2j] );
...
// s3 doesn't contain the helix penalty
{
    ...
    for (l = h+1; l <= j; l++)
    {
        ...
        IFD
            ...
            s3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
        ...
        if (l+2 < j)
        {
            ...
            IFD
                ...
                s3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                        ( exp_dangle3 (l, h, l+1) * u1[lp2j]*EXPC(1) + u1nod5[lp1j] - EXPC(1)*u1nod5[lp2j] );
        }
    }
}


void s_partition_function::compute_s3nod5 (int h, int j)
// s3 doesn't contain the helix penalty
{
    if (h < 1) return;
    IFD return;
    ...
    {
        hl = index[h] + l - h;
        s3nod5[hj] += up[hl] * exp_AUpenalty (h, l) *
                  ( fd3 (j+1, h, l) * exp ((j-l) * misc.multi_free_base_penalty * oneoverRT) );
...
void s_partition_function::compute_s3nod3 (int h, int j)
// s3 doesn't contain the helix penalty
{
    if (h < 1) return;
    IFD return;
    ...
    {
        hl = index[h] + l - h;
        s3nod3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                  ( exp ((j-l) * misc.multi_free_base_penalty * oneoverRT) );
        ...
        if (l+2 < j)
        {
            ...
            lp1j = index[l+1]+j-l-1;
            s3nod3[hj] += up[hl] * exp_dangle5 (l, h, h-1) * exp_AUpenalty (h, l) *
                    ( exp_dangle3 (l, h, l+1) * u1nod3[lp2j]*EXPC(1) + u1nod35[lp1j] - EXPC(1)*u1nod35[lp2j] );
        }
    }
}


void s_partition_function::compute_s3nod35 (int h, int j)
// s3 doesn't contain the helix penalty
{
    if (h < 1) return;
    IFD return;
    ...
    {
        hl = index[h] + l - h;
        s3nod35[hj] += up[hl] * exp_AUpenalty (h, l) *
                  ( exp ((j-l) * misc.multi_free_base_penalty * oneoverRT) );
...
void s_partition_function::print_base_pair_probabilities ()
{
    int i, j;
...
//     else
    {
        if (h > TURN)
        {
            term1 +=  (u_ip_jp[h-1] + u_iu_jp[h-1] + exp_dangle5(l,h,h-1)*(u_ip_ju[h-1] + u_iu_ju[h-1]));
                    //u[h-1] - u[h-2] + unod3[h-2] - unod3[h-3] +   // ..)[...] or ..).[...]      // index[0] is 0
                    //exp_dangle5(l,h,h-1) * (u[h-2] - (unod3[h-2]-unod3[h-3]));    // ..)..[...]
        }
        else if (h > 0)
        {
...
//     else
    {
        if (l < seqlen-3)
        {
            ...
            term2 += (u_ip_jp[lp1n] + u_ip_ju[lp1n] +
                    ...
                    //unod5[index[l+1]+seqlen-1-(l+1)] - unod5[index[l+2]+seqlen-1-(l+2)] +   //[...](...
                    //exp_dangle3(l,h,l+1) * u[index[l+2]+seqlen-1-(l+2)];    //  [...].-(...
        }
        else if (l < seqlen-1)
        {
...
//         IFD
//             p[hl] += up[hl] * exp_AUpenalty (h,l) * EXPA * EXPB(2) * EXPC(h-i-1) * pm[il];
//         else
        {
            if (i < h-1)  // we must add d3 (which is added in pmd3); also add d5 here, if i < h-2
            {
                 p[hl] += up[hl] * EXPA * EXPB(2) * exp_AUpenalty (h,l) *
...
//             IFD
//             {
//                 p[hl] += up[hl] * exp_AUpenalty (h,l) * EXPA * EXPB(2) * pm1[il] * u1[ip1hm1];    // no branch to the right of h-l
//                 p[hl] += up[hl] * exp_AUpenalty (h,l) * EXPA * EXPB(2) * pm[il] * u1[ip1hm1];    // branch to the left and to the right of h-l
//             }
//             else
            {
                ...
                pm1nod3_needendd3[il] * exp_dangle3(l,h,l+1) *    // EXPC added in pm1nod3
                    ...
                    /*
                    (u1nod5[ip1hm1] - EXPC(1)*u1nod5[ip2hm1]
                        - EXPC(1)*u1nod5[ip1hm2] + EXPC(2)*u1nod5[ip2hm2]
                        + EXPC(1)*u1nod35[ip1hm2] - EXPC(2)*u1nod35[ip2hm2]
                        - EXPC(2)*u1nod35[ip1hm3] + EXPC(3) * u1nod35[ip2hm3]
                        // ((..-)[...].-) or ((..-).[...].-)
                        + exp_dangle5(l,h,h-1)*EXPC(1)*
                            (u1nod5[ip1hm2] - EXPC(1)*u1nod5[ip2hm2]
                            - u1nod35[ip1hm2] + EXPC(1)*u1nod35[ip2hm2]
                            + EXPC(1)*u1nod35[ip1hm3] - EXPC(2)*u1nod35[ip2hm3]))    */
                ...
                + pm1d3_needendd3[il] * exp_dangle3(l,h,l+1) *     // don't need EXPC(1)
                    (u1_ip_jp[ip2hm1] + exp_dangle5(l,h,h-1)* u1_ip_ju[ip2hm1])
                        /*(u1nod5[ip2hm1] - EXPC(1)*u1nod5[ip3hm1]
                        - EXPC(1)*u1nod5[ip2hm2] + EXPC(2) * u1nod5[ip3hm2]
                        + EXPC(1)*u1nod35[ip2hm2] - EXPC(2) * u1nod35[ip3hm2]
                        - EXPC(2)*u1nod35[ip2hm3] + EXPC(3) * u1nod35[ip3hm3]
                            // (.(...)[...].-) or (.(...).[...].-)       i.(...)h...lj
                        + exp_dangle5(l,h,h-1)*EXPC(1)*
                            (u1nod5[ip2hm2] - EXPC(1)*u1nod5[ip3hm2]
                            - u1nod35[ip2hm2] + EXPC(1)*u1nod35[ip3hm2]
                            + EXPC(1)*u1nod35[ip2hm3] - EXPC(2)*u1nod35[ip3hm3])) */
                            // (.(...).-[...].-)        i.(...)h...l.-j

                // case (..-(..-)-[...].-)
                + pm1d3_needendd3[il]  * exp_dangle3(l,h,l+1) *     // don't need EXPC(1)
                    (u1_iu_jp[ip2hm1] + exp_dangle5(l,h,h-1)*u1_iu_ju[ip2hm1])
                    /*(u1[ip2hm1] - u1nod5[ip2hm1] + EXPC(1)*u1nod5[ip3hm1]
                            - EXPC(1)*u1[ip2hm2] + EXPC(1)*u1nod5[ip2hm2] - EXPC(2)*u1nod5[ip3hm2]
                            + EXPC(1)*u1nod3[ip2hm2] - EXPC(1)*u1nod35[ip2hm2] + EXPC(2)*u1nod35[ip3hm2]
                            - EXPC(2)*u1nod3[ip2hm3] + EXPC(2)*u1nod35[ip2hm3] - EXPC(3)*u1nod35[ip3hm3]
                            // (..-(...)[...]) or (..-(...).[...])       i.(...)h...lj
                            + exp_dangle5(l,h,h-1)*EXPC(1)*
                            (u1[ip2hm2] - u1nod5[ip2hm2] + EXPC(1)*u1nod5[ip3hm2]
                            - u1nod3[ip2hm2] + u1nod35[ip2hm2] - EXPC(1)*u1nod35[ip3hm2]
                            + EXPC(1)*u1nod3[ip2hm3] - EXPC(1)*u1nod35[ip2hm3] + EXPC(2)*u1nod35[ip3hm3])) */
                            // (..-(...)..-[...].-)      i..-(...)h...l.-j
    ...
    for (j = l+TURN+3; j < seqlen; j++)
    {
        if (can_pair (sequence[i], sequence[j]))
        {
            ...
            pmd3_needmidd3[il] += exp_AUpenalty (i,j) * exp_dangle3(i,j,i+1) * p[ij] / up[ij] * // don't need to add EXPC(1) here, it's added in the calling function
                            ( u1_ip_jp[lp2jm1] + u1_iu_jp[lp2jm1]      // ..))  or ..).)
                                + exp_dangle5(i,j,j-1) * (u1_ip_ju[lp2jm1] + u1_iu_ju[lp2jm1])  );    //)-..)
//                             ( u1[lp2jm1] - EXPC(1)*u1[lp2jm2]
//                                 + EXPC(1)*u1nod3[lp2jm2] - EXPC(2)*u1nod3[lp2jm3] +
//                               ( u1[lp2jm2] - u1nod3[lp2jm2] + EXPC(1)*u1nod3[lp2jm3]) *
//                                   exp_dangle5(i,j,j-1) * EXPC(1) );
        ...
        if (can_pair (sequence[i], sequence[j]))
        {
            ...
            pmd3_noneedmidd3[il] += exp_AUpenalty (i,j) * exp_dangle3(i,j,i+1) * p[ij] / up[ij] * // shouldn't add EXPC(1) here because it's in the calling function
                            (u1_ip_jp[lp1jm1] + exp_dangle5(i,j,j-1) * u1_ip_ju[lp1jm1]);
                                // ..))  or ..).),  and then )-..)
                            /*
                            ( u1nod5[lp1jm1] - EXPC(1)*u1nod5[lp2jm1]
                                  - EXPC(1)*u1nod5[lp1jm2] + EXPC(2)*u1nod5[lp2jm2]
                                  + EXPC(1)*u1nod35[lp1jm2] - EXPC(2)*u1nod35[lp2jm2]
                                  - EXPC(2)*u1nod35[lp1jm3] + EXPC(3)*u1nod35[lp2jm3] +    // ..))  or ..).)
                              ( u1nod5[lp1jm2] - EXPC(1)*u1nod5[lp2jm2]
                                  - u1nod35[lp1jm2] + EXPC(1)*u1nod35[lp2jm2]
                                  + EXPC(1)*u1nod35[lp1jm3] - EXPC(2)*u1nod35[lp2jm3]) *
                                  exp_dangle5(i,j,j-1) * EXPC(1) );    */
...
/*                            ( u1[lp2jm1] - EXPC(1)*u1[lp2jm2] +
                              EXPC(1)*u1nod3[lp2jm2] - EXPC(2)*u1nod3[lp2jm3] +     // ..))  or ..).)
                              ( u1[lp2jm2] - u1nod3[lp2jm2] + EXPC(1)*u1nod3[lp2jm3]) *
                                  exp_dangle5(i,j,j-1) * EXPC(1));*/
        }
    }
        ...
        if (can_pair (sequence[i], sequence[j]))
        {
            ...
            pm2d5_needmidd5[hj] += p[ij] / up[ij] * exp_AUpenalty (i,j) * exp_dangle5(i,j,j-1) *
                ...
                ( (u1nod5[ip1hm2] - EXPC(1) * u1nod5[ip2hm2]
                    ...
                    + EXPC(1)*u1nod35[ip1hm3] - EXPC(2) * u1nod35[ip2hm3])
                   // next, (.-(---)-..[...].-.)
                   + exp_dangle3(i,j,i+1) * EXPC(1) * (u1[ip2hm2] - u1nod3[ip2hm2] + EXPC(1)*u1nod3[ip2hm3]));
                */
        }
    }
}
            ...
            pm2d5_noneedmidd5[hj] += p[ij] / up[ij] * exp_AUpenalty (i,j) * exp_dangle5(i,j,j-1) *
                ...
                /*
                  (u1nod5[ip1hm1] - EXPC(1) * u1nod5[ip2hm1]
                    ...
                    - EXPC(2)*u1nod35[ip1hm3] + EXPC(3) * u1nod35[ip2hm3]
                   // next, (.-(---)[...].-.) or (.-(---).[...].-.)
                   + exp_dangle3(i,j,i+1) * EXPC(1) *
                       (u1[ip2hm1] - EXPC(1)*u1[ip2hm2] + EXPC(1)*u1nod3[ip2hm2] - EXPC(2)*u1nod3[ip2hm3]) );
                */
        }
    }
}
...
//         printf ("\t++++++++++++++++++++++\n");
    ...
    for (i=0; i < seqlen; i++)
        for (j=i+1; j < seqlen; j++)
        {
            ...
            double unew = u_ip_jp[ij] + u_ip_ju[ij] + u_iu_jp[ij] + u_iu_ju[ij];
...
// u, up and p arrays are filled
{
                            ...
                            for (jj = ii+TURN+1; jj < seqlen; jj++)
                            {
                                    ...
                                    sequence[ii+1] == k && sequence[jj-1] == l)
                                {
                                    ...
                                    if (jj-ii-1 == 3)
                                    {
                                        ...
                                        {
                                            sizeindex = structure_type_index ("misc.terminal_AU_penalty");
                                            GlogZ[sizeindex] += p[iijj] / up[iijj] * exp (en_hairpin * oneoverRT);
                                        }
                                    }
                                        ...
                                        for (jjp = minq; jjp < jj; jjp++)
                                        {
                                                ...
                                                sequence[iip]+sequence[jjp] == 5)
                                            {
                                                ...
                                                if (branch1 == 0 || branch2 == 0)    // bulge
                                                {
                                                    ...
                                                    {
                                                        sprintf (s, "misc.terminal_AU_penalty");
                                                        // this is very slow, it's linear in the number of parameters
                                                        int sizeindex = structure_type_index (s);
                                                        if (has_AU_penalty (sequence[ii], sequence[jj]))
                                                        {
    ...
    if (fix_dangles)    // we don't need to compute dangling ends
    {
                            ...
                            if (sequence[jj] == i && sequence[ii] == j && sequence[jj+1] == k)
                            {
                                ...
                                if (ii > TURN)
                                    term1 += u_ip_jp[ii-1] + u_iu_jp[ii-1] + exp_dangle5(jj,ii,ii-1)*(u_ip_ju[ii-1] + u_iu_ju[ii-1]);
                                            // we have to add the contribution of d3
                                            //u[ii-1] - u[ii-2] + unod3[ii-2] - unod3[ii-3] +     // index[0] is 0
                                            //exp_dangle5(jj,ii,ii-1) * (u[ii-2] - (unod3[ii-2]-unod3[ii-3]));
                                            //u[index[0]+ii-1] - u[index[0]+ii-3] + //?? unod[ii-3]
                                            //exp_dangle5(jj,ii,ii-1) * u[index[0]+ii-3];
                                ...
                                if (jj < seqlen-3)
                                {
                                    ...
                                    term2 +=
                                    //u_ip_jp[lp1n] + u_ip_ju[lp1n] +
                        //exp_dangle3(l,h,l+1)*(u_ip_jp[lp2n] + u_ip_ju[lp2n] + u_iu_jp[lp2n] + u_iu_ju[lp2n]);
                                            // ignore the case when d3 is not involved
                                            //unod[index[jj+1]+seqlen-1-(jj+1)] - unod[index[jj+2]+seqlen-1-(jj+2)] +
                            ...
                            if (sequence[ii] == i && sequence[jj] == j && sequence[ii+1] == k)
                            {
                                ...
                                for (l=ii+3; l < jj-TURN-2; l++)    // case (.(...)--(--)-)
                                {
                                    ...
                                    temp += exp_AUpenalty (ii,jj) * up[iip2l] *
                                            ...
                                            /*
                                            ( exp_dangle3 (l, ii+2, l+1) * EXPC(1)*
                                                    (u1[lp2jjm1] - EXPC(1)*u1[lp2jjm2] +
                                                    EXPC(1)*u1nod3[lp2jjm2] - EXPC(2)*u1nod3[lp2jjm3] + // (.(...).-(...)) or ((...).-(...).)
                                                    (u1[lp2jjm2] - u1nod3[lp2jjm2] + EXPC(1)*u1nod3[lp2jjm3]) *
                                                        exp_dangle5(ii,jj,jj-1)*EXPC(1)) +      // (.(...).-(...)-..)
                                                (u1nod5[lp1jjm1] - EXPC(1)*u1nod5[lp1jjm2]
                                                    - EXPC(1)*u1nod5[lp2jjm1] + EXPC(2)*u1nod5[lp2jjm2]
                                                    + EXPC(1)*u1nod35[lp1jjm2] - EXPC(2)*u1nod35[lp1jjm3]
                                                    - EXPC(2)*u1nod35[lp2jjm2] + EXPC(3)*u1nod35[lp2jjm3]) + // (.(...)(.-..)) or ((...)(.-..).)
                                                (u1nod5[lp1jjm2] - EXPC(1)*u1nod5[lp2jjm2]
                                                    - u1nod35[lp1jjm2] + EXPC(1)*u1nod35[lp2jjm2]
                                                    + EXPC(1)*u1nod35[lp1jjm3] - EXPC(2)*u1nod35[lp2jjm3]) *
                                                    exp_dangle5(ii,jj,jj-1)*EXPC(1));   // (.(...)(...)-..)
                                            */
                                }
                                double upm_temp = 0;
                                for (h=ii+3; h < jj-TURN-2; h++)    // case (....(...)--(--)-)
                                {
                                    ...
                                    upm_temp += EXPA * EXPB(2) * EXPC(h-ii-1) *
                                        ( s2_jp[hjjm1]    // --(...)) or --(...).)
                                          ...
                                          /*
                                        (s2[hjjm1] - EXPC(1)*s2[hjjm2]
                                            + EXPC(1)*s2nod3[hjjm2] - EXPC(2)*s2nod3[hjjm3] + // --(...)) or --(...).)
                                            (s2[hjjm2] - s2nod3[hjjm2] + EXPC(1)*s2nod3[hjjm3]) * exp_dangle5(ii,jj,jj-1)*EXPC(1));    // --(...)..)
                                        */
                                    //s2[hj];
                                }
                                ...
                                sequence[l] == i && sequence[h] == j && sequence[l+1] == k)
                            {
                                for (ii=0; ii < h; ii++)
                                {
                                    ...
                                    if (ii < h - TURN -2)    // only now we can have a branch to the left
                                    {
                                        ...
                                        /*
                                            (u1nod5[iip1hm1] - EXPC(1) * u1nod5[iip2hm1]
                                                - EXPC(1)*u1nod5[iip1hm2] + EXPC(2) * u1nod5[iip2hm2]
                                                + EXPC(1)*u1nod35[iip1hm2] - EXPC(2) * u1nod35[iip2hm2]
                                                - EXPC(2)*u1nod35[iip1hm3] + EXPC(3) * u1nod35[iip2hm3]
                                                // ((..-)[...].-) or ((..-).[...].-)
                                                + exp_dangle5(l,h,h-1)*EXPC(1)*
                                                    (u1nod5[iip1hm2] - EXPC(1) * u1nod5[iip2hm2]
                                                    - u1nod35[iip1hm2] + EXPC(1) * u1nod35[iip2hm2]
                                                    + EXPC(1)*u1nod35[iip1hm3] - EXPC(2) * u1nod35[iip2hm3]))
                                        */
                                                    // c is added in pm1nod3_needendd3
                                                    // ((...).-[...].-)         i(...)h...l.-j
                                        // case (.(..-)-[...].-)
                                        + pm1d3_needendd3[iil] * exp_dangle3(l,h,l+1) * //  don't need EXPC(1)
                                            (u1_ip_jp[iip2hm1] + exp_dangle5(l,h,h-1)*u1_ip_ju[iip2hm1])
                                            /*
                                                (u1nod5[iip2hm1] - EXPC(1) * u1nod5[iip3hm1]
                                                - EXPC(1)*u1nod5[iip2hm2] + EXPC(2) * u1nod5[iip3hm2]
                                                + EXPC(1)*u1nod35[iip2hm2] - EXPC(2) * u1nod35[iip3hm2]
                                                - EXPC(2)*u1nod35[iip2hm3] + EXPC(3) * u1nod35[iip3hm3]
                                                    // (.(...)[...].-) or (.(...).[...].-)       i.(...)h...lj
                                                + exp_dangle5(l,h,h-1)*EXPC(1)*
                                                    (u1nod5[iip2hm2] - EXPC(1) * u1nod5[iip3hm2]
                                                    - u1nod35[iip2hm2] + EXPC(1) * u1nod35[iip3hm2]
                                                    + EXPC(1)*u1nod35[iip2hm3] - EXPC(2) * u1nod35[iip3hm3]))  */
                                                    // (.(...).-[...].-)        i.(...)h...l.-j

                                        // case (..-(..-)-[...].-)
                                        + pm1d3_needendd3[iil]  * exp_dangle3(l,h,l+1) *     // don't need EXPC(1)
                                            (u1_iu_jp[iip2hm1] + exp_dangle5(l,h,h-1)*u1_iu_ju[iip2hm1])
                                            /*
                                            (u1[iip2hm1] - u1nod5[iip2hm1] + EXPC(1) * u1nod5[iip3hm1]
                                                 - EXPC(1)*u1[iip2hm2] + EXPC(1)*u1nod5[iip2hm2] - EXPC(2) * u1nod5[iip3hm2]
                                                 + EXPC(1)*u1nod3[iip2hm2] - EXPC(1)*u1nod35[iip2hm2] + EXPC(2) * u1nod35[iip3hm2]
                                                 - EXPC(2)*u1nod3[iip2hm3] + EXPC(2)*u1nod35[iip2hm3] - EXPC(3) * u1nod35[iip3hm3]
                                                    // (.(...)[...]) or (.(...).[...])       i.(...)h...lj
                                                 + exp_dangle5(l,h,h-1)*EXPC(1)*
                                                 (u1[iip2hm2] - u1nod5[iip2hm2] + EXPC(1) * u1nod5[iip3hm2]
                                                 - u1nod3[iip2hm2] + u1nod35[iip2hm2] - EXPC(1) * u1nod35[iip3hm2]
                                                 + EXPC(1)*u1nod3[iip2hm3] - EXPC(1)*u1nod35[iip2hm3] + EXPC(2) * u1nod35[iip3hm3])) */
                                                    // (..-(...)[...].-)      i..-(...)h...l.-j
                            ...
                            if (sequence[jj] == i && sequence[ii] == j && sequence[ii-1] == k)
                            {
                                ...
                                if (ii > TURN)
                                    ...
                                    term1 += exp_dangle5(jj,ii,ii-1)*(u_ip_ju[ii-1] + u_iu_ju[ii-1]);
                                            // ignore the case when d5 is not involved
                                            //(u[index[0]+ii-1] - unod[index[0]+ii-3]) +
                                            //exp_dangle5(jj,ii,ii-1) * (u[ii-2]-(unod3[ii-2]-unod3[ii-3]));  // index[0] is 0
                                else if (ii > 0)      term1 += exp_dangle5(jj,ii,ii-1);
                                ...
                                if (jj < seqlen-3)
                                {
                                    ...
                                    term2 += (u_ip_jp[jjp1n] + u_ip_ju[jjp1n] +
                                        exp_dangle3(jj,ii,jj+1)*(u_ip_jp[jjp2n] + u_ip_ju[jjp2n] + u_iu_jp[jjp2n] + u_iu_ju[jjp2n]));
                                            ...
                                            //unod5[index[jj+1]+seqlen-1-(jj+1)] - unod5[index[jj+2]+seqlen-1-(jj+2)] +
                                            //exp_dangle3(jj,ii,jj+1) * u[index[jj+2]+seqlen-1-(jj+2)];
                                else if (jj < seqlen-1) term2 += exp_dangle3(jj,ii,jj+1);
                                else                  term2 = 1;

                                //printf ("ii=%d, jj=%d, %g\n", ii, jj, term1 * up[iijj] * term2 / u[index[0]+seqlen-1] * exp_AUpenalty (ii,jj));
                                GlogZ[index_param] += term1 * up[iijj] * term2 * exp_AUpenalty (ii,jj) /
                                    (u_ip_jp[seqlen-1] + u_ip_ju[seqlen-1] + u_iu_jp[seqlen-1] + u_iu_ju[seqlen-1]);
                        ...
                        for (l=seqlen-1; l > h+TURN; l--)
                        {
                                ...
                                sequence[l] == i && sequence[h] == j && sequence[h-1] == k)
                            {
                                ...
                                for (ii=0; ii < h; ii++)
                                {
                                    ...
                                    {
                                        GlogZ[index_param] += up[hl] * EXPA * EXPB(2) * exp_AUpenalty (h,l) *
                                            EXPC(h-ii-1) * pmd3_noneedmidd3[iil] * exp_dangle5(l,h,h-1);
                                            // (.-[...](---)-)    i.-h...l(---)-j
                                        GlogZ[index_param] += up[hl] * EXPA * EXPB(2) * exp_AUpenalty (h,l) *
                                            EXPC(h-ii-1) * pmd3_needmidd3[iil]  * exp_dangle3(l,h,l+1) * EXPC(1) * exp_dangle5(l,h,h-1);
                                            // (.-[...].-(---)-)    i.-h...l.-(---)-j
                                    }
...
//                                                     + EXPC(1)*u1nod35[iip1hm3] - EXPC(2) * u1nod35[iip2hm3]))
                                        ...
                                        // case (.(..-)-[...].-)
                                        + pm1d3_needendd3[iil] * exp_dangle3(l,h,l+1) *     // don't need EXPC(1)
                                            (exp_dangle5(l,h,h-1)*u1_ip_ju[iip2hm1])
//                                                 (exp_dangle5(l,h,h-1)*EXPC(1)*
//                                                     (u1nod5[iip2hm2] - EXPC(1) * u1nod5[iip3hm2]
//                                                     - u1nod35[iip2hm2] + EXPC(1) * u1nod35[iip3hm2]
//                                                     + EXPC(1)*u1nod35[iip2hm3] - EXPC(2) * u1nod35[iip3hm3]))
                                        ...
                                        // case (..-(..-)-[...].-)
                                        + pm1d3_needendd3[iil]  * exp_dangle3(l,h,l+1) *     // don't need EXPC(1)
                                            (exp_dangle5(l,h,h-1)*u1_iu_ju[iip2hm1])
//                                             (exp_dangle5(l,h,h-1)*EXPC(1)*
//                                                     (u1[iip2hm2] - u1nod5[iip2hm2] + EXPC(1) * u1nod5[iip3hm2]
//                                                     - u1nod3[iip2hm2] + u1nod35[iip2hm2] - EXPC(1) * u1nod35[iip3hm2]
//                                                     + EXPC(1)*u1nod3[iip2hm3] - EXPC(1)*u1nod35[iip2hm3] + EXPC(2) * u1nod35[iip3hm3]))
...
//                 else
                {
                    if (ii > TURN)
                            // case i-1 paired
                        term1 += (u_ip_jp[ii-1] + u_iu_jp[ii-1] + exp_dangle5(jj,ii,ii-1)*(u_ip_ju[ii-1] + u_iu_ju[ii-1]));
                                //u[ii-1] - u[ii-2] + unod3[ii-2] - unod3[ii-3] +
                                //exp_dangle5(jj,ii,ii-1) * (u[ii-2] - unod3[ii-2] + unod3[ii-3]);
                    else if (ii > 0)      term1 += exp_dangle5(jj,ii,ii-1);
                    else                  term1 = 1;
                }
...
//                             (exp_dangle3 (i, j, i+1) * EXPC(1)*
//                               (u1[ip2km1] - u1nod3[ip2km1] + EXPC(1)*u1nod3[ip2km2]) // (.(---).k(
//                             + u1nod5[ip1km1] - u1nod35[ip1km1] + EXPC(1)*u1nod35[ip1km2]
//                             - EXPC(1)*u1nod5[ip2km1] + EXPC(1)*u1nod35[ip2km1] - EXPC(2)*u1nod35[ip2km2]) *   // ((---).k(
//                             // the branch(es) to the right of k
//                             (u1[kjm1] - EXPC(1)*u1[kjm2] + EXPC(1)*u1nod3[kjm2] - EXPC(2)*u1nod3[kjm3] + // --).k(...)) or --).k(...).)
//                                     (u1[kjm2] - u1nod3[kjm2] + EXPC(1)*u1nod3[kjm3]) * exp_dangle5(i,j,j-1)*EXPC(1)  // ((...)k-(...)-..)
//                                 - (u1nod5[kjm1] - EXPC(1)*u1nod5[kjm2] + EXPC(1)*u1nod35[kjm2] - EXPC(2)*u1nod35[kjm3] +
//                                     (u1nod5[kjm2] - u1nod35[kjm2] + EXPC(1)*u1nod35[kjm3]) * exp_dangle5(i,j,j-1)*EXPC(1))
//                                 + EXPC(1)*(u1nod5[kp1jm1] - EXPC(1)*u1nod5[kp1jm2]
//                                             + EXPC(1)*u1nod35[kp1jm2] - EXPC(2)*u1nod35[kp1jm3] +
//                                            (u1nod5[kp1jm2] - u1nod35[kp1jm2] + EXPC(1)*u1nod35[kp1jm3]) * exp_dangle5(i,j,j-1)*EXPC(1)));
...
//                             ( exp_dangle3 (i, j, i+1) * EXPC(1)*
//                                  (u1[ip2k] - u1nod3[ip2k] + EXPC(1)*u1nod3[ip2km1]
//                                    - EXPC(1)*u1[ip2km1] + EXPC(1)*u1nod3[ip2km1] - EXPC(2)*u1nod3[ip2km2]) // (.-(--)k
//                                 + u1nod5[ip1k] - u1nod35[ip1k] + EXPC(1)*u1nod35[ip1km1]
//                                   - EXPC(1)*u1nod5[ip1km1] + EXPC(1)*u1nod35[ip1km1] - EXPC(2)*u1nod35[ip1km2]
//                                 - EXPC(1)*(u1nod5[ip2k] - u1nod35[ip2k] + EXPC(1)*u1nod35[ip2km1]
//                                          - EXPC(1)*u1nod5[ip2km1] + EXPC(1)*u1nod35[ip2km1] - EXPC(2)*u1nod35[ip2km2])); //*     // ((---)k
//
//                             // the branch(es) to the right of k
//                             (u1[kp1jm1] - EXPC(1)*u1[kp1jm2] + EXPC(1)*u1nod3[kp1jm2] - EXPC(2)*u1nod3[kp1jm3] + // ((...)k-(...)) or ((...)k-(...).)
//                               (u1[kp1jm2] - u1nod3[kp1jm2] + EXPC(1)*u1nod3[kp1jm3]) * exp_dangle5(i,j,j-1)*EXPC(1));  // ((...)k-(...)-..)
                    }
                }
            }
        }
...
//             else
    ...
    for (i=0; i < num_params; i++)
    {
        if (GlogZ[i] > 1)
        {
            fprintf (stderr, "ERROR: Gradient GlogZ[%d] too large!! It is %g\n", i, GlogZ[i]);
            //exit(1);
        }
    }
    */
